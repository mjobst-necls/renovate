...
import { cacheSubDir, packageKeys, requiredPackageKeys } from './common.ts';
import { extract } from './file.ts';
import { formatReleaseResult } from './release.ts';
...
export class ApkDatasource extends Datasource {
  ...
  private async downloadAndExtractIndexArchive(
    ...
    const compressedFile = upath.join(
      fullCacheDir,
      `${randomUUID()}_${indexUrlHash}.${compression}`,
    );
