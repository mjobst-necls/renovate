...
const apkBaseUrl = 'https://dl-cdn.alpinelinux.org/alpine';

///v3.23/main/x86_64/

describe('modules/datasource/apk/index', () => {
  const fixturePackagesArchivePath = Fixtures.getPath(`APKINDEX.tar.gz`);
  ...
  let apkDatasource: ApkDatasource;
  ...
  beforeEach(async () => {
    apkDatasource = new ApkDatasource();
    ...
    extractedPackageFile = upath.join(
      extractionFolder,
      `${toSha256(getComponentUrl(apkBaseUrl, 'stable', 'non-free', 'amd64'))}.txt`,
    );
    ...
    cfg = {
      ...
      packageName: 'album',
      registryUrls: [
        getRegistryUrl(apkBaseUrl, 'stable', ['non-free'], 'amd64'),
      ],
    };
